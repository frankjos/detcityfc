...
abstract class Fishpig_Wordpress_Model_Resource_Post_Collection_Abstract extends Fishpig_Wordpress_Model_Resource_Collection_Abstract
{
	...
	{
		if (!$this->getFlag('post_type_filter_exists')) {
			$this->addPostTypeFilter($this->getNewEmptyItem()->getPostType());
		}
	...
	{
		$this->setFlag('post_type_filter_exists', true);
		
		$op = is_array($postTypes) ? 'in' : 'eq';
		
		return $this->addFieldToFilter('post_type', array($op => $postTypes));
	}
